...
<button class="float-right p-2 mb-2 btn btn-sm btn-primary" style="margin-top:10px"
    onclick="window.location='{{ route('activity.create') }}'">Add
    Activity</button>
@endcan
@endsection
@section('content')
<div class="container-fluid">
    <div class="fade-in">
        <div class="col-lg-12">
            ...
            <div class="card">
                <div class="card-header"><i class="fa fa-align-justify"></i> Activity</div>
                <div class="card-body">
                    <table class="table table-responsive-sm table-bordered table-striped table-sm">
                        ...
                        <tbody class="row_drag">
                            @forelse ($activitys as $key=>$activity)
                            <tr data-id="{{$activity->id}}">

                                <td>{{ $activity->title }}</td>
                                <td>
                                    {{ Str::limit(html_entity_decode(strip_tags($activity->description)),100,'......') }}
                                </td>


                                <td>
                                    <img src="{{Storage::url($activity->image)}}" height="100">
                                </td>

                                <td>{!! $activity->publish?'<span
                                        class="badge badge-pill badge-success">Active</span>':'<span
                                        class="badge badge-pill badge-warning">Inactive</span>' !!}</td>
                                <td>
                                    <button class="btn btn-xs btn-info"
                                    onclick="window.location=`{{ route('activity.edit',['id'=>$activity->id]) }}`">Edit</button>
                                    <button data-question="Are you sure to delete the data?" data-toggle="confirm" data-id="{{ $activity->id }}" class="btn btn-xs btn-danger">Delete</button>
                                </td>
                            </tr>
                            @empty
                            <tr>
...
<script>
    $(function(){
        $('[data-toggle="confirm"]').jConfirm().on('confirm', function(e){
            var btn = $(this),
            id = btn.data('id');
            var url = '{{ route("activity.delete", ":id") }}';
    ...
    $( ".row_drag" ).sortable({
        ...
        stop: function() {
           ...
           $.ajax({
              ...
              url:"{{route('activity.update.order')}}",
           });
        }
    });
</script> --}}

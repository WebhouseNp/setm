...
<ol class="m-0 border-0 breadcrumb">
    ...
    <li class="breadcrumb-item"><a href="{{ route('quicklink.index') }}">Quick Link</a></li>

    {{-- <li class="breadcrumb-item active">Dashboard</li> --}}
</ol>
@can('Add Quicklink')
<button class="float-right p-2 mb-2 btn btn-sm btn-primary" style="margin-top:10px"
    onclick="window.location='{{ route('quicklink.create') }}'">Add
    Quick Link</button>
@endcan
@endsection
@section('content')
<div class="container-fluid">
    <div class="fade-in">
        <div class="col-lg-12">
            ...
            <div class="card">
                <div class="card-header"><i class="fa fa-align-justify"></i> Quick Link</div>
                <div class="card-body">
                    <table class="table table-responsive-sm table-bordered table-striped table-sm">
                        ...
                        <tbody class="row_drag">
                            @forelse ($quicklinks as $key=>$quicklink)
                            <tr data-id="{{$quicklink->id}}">

                                <td>{{ $quicklink->title }}</td>
                                <td>{{ $quicklink->link }}</td>
                                <td>{!! $quicklink->publish?'<span
                                        class="badge badge-pill badge-success">Active</span>':'<span
                                        class="badge badge-pill badge-warning">Inactive</span>' !!}</td>
                                <td>
                                    <button class="btn btn-xs btn-info"
                                    onclick="window.location=`{{ route('quicklink.edit',['id'=>$quicklink->id]) }}`">Edit</button>
                                    <button data-question="Are you sure to delete the data?" data-toggle="confirm" data-id="{{ $quicklink->id }}" class="btn btn-xs btn-danger">Delete</button>
                                </td>
                            </tr>
                            @empty
                            <tr>
                    ...
                    {{ $quicklinks->links('pagination::bootstrap-4') }}
                </div>
...
<script>
    $(function(){
        $('[data-toggle="confirm"]').jConfirm().on('confirm', function(e){
            var btn = $(this),
            id = btn.data('id');
            var url = '{{ route("quicklink.delete", ":id") }}';
    ...
    $( ".row_drag" ).sortable({
        ...
        stop: function() {
           ...
           $.ajax({
              ...
              url:"{{route('quicklink.update.order')}}",
           });
        }
    });
</script>
